...
interface ChartPreviewProps {
  ...
  externalExportRequest?: { type: "png" | "svg"; token: number } | null;
...
function buildHistogramOption(data: ParsedData, config: ChartConfig): EChartsOption {
  ...
  if (!values.length) {
    return {
      ...
      title: {
        ...
        left: "center",
        top: 24,
        textStyle: { fontFamily: config.fontFamily, fontSize: config.fontSize },
      },
    };
  }
  ...
  return {
    backgroundColor: config.backgroundColor,
    animation: false,
    grid: { top: 48, left: 56, right: 24, bottom: 52, containLabel: true },
    ...
    series: [
      {
        ...
        data: bins,
        barMaxWidth: 36,
        itemStyle: { color: config.colors[0], borderRadius: [8, 8, 0, 0] },
      },
    ],
  };
}
...
export const ChartPreview = memo(function ChartPreview({
  ...
  mode = "workspace",
  externalExportRequest = null,
  onApplyRecommendation,
}: ChartPreviewProps) {
  const chartRef = useRef<ReactEChartsCore>(null);
  const previousExportTokenRef = useRef<number | null>(null);

  const option = useMemo<EChartsOption>(() => {
    if (config.chartType === "histogram") return buildHistogramOption(data, config);
  ...
  const compactLayout = !isEditorMode && recommendations.length <= 4;

  const containerStyle = useMemo(() => {
    if (config.aspectRatio === "free") return { width: "100%", height: "100%" };
    const [width, height] = config.aspectRatio.split(":").map(Number);
    return { width: "100%", height: "auto", aspectRatio: `${width}/${height}`, maxWidth: "100%" };
  }, [config.aspectRatio]);
  ...
  const exportPNG = useCallback(() => {
    ...
    if (!instance) return;
    const url = instance.getDataURL({ type: "png", pixelRatio: 3, backgroundColor: config.backgroundColor });
    const anchor = document.createElement("a");
    anchor.href = url;
  ...
  useEffect(() => {
    const token = externalExportRequest?.token ?? null;
    if (token === null) {
      previousExportTokenRef.current = null;
      return;
    }
    if (previousExportTokenRef.current === token) return;
    previousExportTokenRef.current = token;
    if (externalExportRequest?.type === "svg") {
      exportSVG();
      return;
    }
    if (externalExportRequest?.type === "png") exportPNG();
  }, [externalExportRequest?.token, externalExportRequest?.type, exportPNG, exportSVG]);
        ...
        <div className="space-y-2">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="text-2xl font-semibold tracking-tight text-slate-950">{config.title || "图表预览"}</h2>
              {autoGeneratedTitle ? (
